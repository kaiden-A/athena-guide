      ...
      <div className="flex-1 flex items-center justify-center px-4 py-6">
        <div className="flex w-full max-w-5xl h-[80vh] bg-[rgba(18,19,35,0.65)] backdrop-blur-xl border border-white/[0.06] rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
          ...
          <main className="flex flex-col flex-1 h-full bg-[rgba(15,16,32,0.2)] min-w-0 relative">
            ...
            {/* Undo Toast */}
            <div className={`absolute top-[72px] left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-2.5 rounded-xl bg-slate-800 border border-white/10 shadow-xl text-sm text-slate-200 transition-all duration-300 ${toast ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
              <span>Conversation cleared</span>
              <button onClick={handleUndo} className="text-sky-400 hover:text-sky-300 font-medium transition-colors">Undo</button>
            </div>

            {/* Messages Content Panel */}
            <section ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-5 scroll-smooth custom-scrollbar">
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-3 max-w-[75%] animate-reveal ${msg.isUser ? 'self-end flex-row-reverse' : 'self-start'}`}>
                  <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold overflow-hidden ${msg.isUser ? 'bg-sky-600 text-white' : 'bg-slate-800'}`}>
                    {msg.isUser ? 'U' : <img src="/athena_logo_v3.svg" alt="Athena" className="w-full h-full object-cover" />}
                  </div>
                  <div className={`flex flex-col gap-1 ${msg.isUser ? 'items-end' : 'items-start'} w-full`}>
                    <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed border border-white/[0.03] shadow-md w-full overflow-x-auto custom-scrollbar ${
            ...
            <footer className="px-6 py-5 bg-[rgba(12,13,26,0.4)] border-t border-white/[0.06] shrink-0">
              ...
              <p className="text-[10px] text-center mt-4 text-slate-600 uppercase tracking-[0.2em] font-medium opacity-60">GUIDED BY MOTION-U INTELLIGENCE</p>
            </footer>
          </main>
        </div>
      </div>
